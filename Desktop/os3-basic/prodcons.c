...
// producers wait here if buffer is full or if it is not their turn yet
...
static int buf_tail = 0;   // next read position
static int buf_count = 0;  // number of items currently in the buffer

// next item that must be inserted to keep ascending order
...
producer (void * arg)
{
    ...
    while (true /* TODO: not all items produced */)
    {
        ...
        // wait until:
        // 1. there is space in the buffer
        // 2. this is the next item that should be produced
